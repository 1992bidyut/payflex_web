...
class SendSMS extends CI_Controller
{
    ...
    public function smsStore($id){
        ...
        else
        {
			...
			$userId = $this->session->userdata('panelSMSOwnerId');

            $i=0;
            foreach($posts['contact_text']  as $smsData){
				
				// MAKING USER ID IN 4 DIGIT 0000
				$userIdWithPrefix    = str_pad($userId, 4, '0', STR_PAD_LEFT);

				// MAKING LOOP  IN 4 DIGIT 0000
				$loopCount           = str_pad($i, 6, '0', STR_PAD_LEFT);
                ...
                $SMSArray[$i]['trx_id']         = $userIdWithPrefix.$loopCount.uniqid();
...
public function finalizedExcelSmsArray(){
    ...
    public function smsExcelStoreConfirm(){
        ...
        $userId = $this->session->userdata('excelOnerId');
        ...
        {
            $this->session->set_flashdata('error_msg','Please try again. Connection was slow!');
            redirect(base_url('SendSMS/sendSMSByExcel/').$userId);
...
//            $MyFile = file_get_contents(base_url()."application/controllers/readme.txt");
            ...
            if($batchInsertID)
            {
                $this->session->set_flashdata('success_msg','Please Confirm !');
	...
	{
		$userId = $this->session->userdata('textOnerId');
        $lastTextFileName = $this->session->userdata('lastTextFileName');
		...
		{
			$this->session->set_flashdata('error_msg','Please try again. Connection was slow!');
            redirect(base_url('SendSMS/sendSMSFromText/').$userId);
		}
